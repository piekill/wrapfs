			   ...
			   size_t count, loff_t *ppos)
{
	...
	lower_file = wrapfs_lower_file(file);
    	/* if no address space operations, use vfs_read;
	   else use do_sync_read
	 */
    	if (MMAP_FLAG == 0)
...
#ifdef WRAPFS_CRYPTO
		/*
		  ioctl used to set key for wrapfs crypto.
		  WRITE_IOCTL : set key and init cipher;
		  RESET_IOCTL : reset key to 0, cipher to NULL;
		 */
		case WRITE_IOCTL:
	    		err = copy_from_user(WRAPFS_SB(sb)->key, (char *)arg, AES_KEYLEN);
			if (err)
			    goto out;
			err = init_cipher(WRAPFS_SB(sb)->key);
	    		break;
	    	case RESET_IOCTL:
			memset(WRAPFS_SB(sb)->key, 0, AES_KEYLEN);
			desc = NULL;
			err = 0;
			break;
#endif
...
static int wrapfs_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	/* if address space operations indicated, use generic_file_mmap(file, vma);
	   else use vm_ops.
	 ...
	 */
	    ...
	    if (err)
		printk(KERN_ERR
		       "wrapfs: generic_file_mmap failed %d\n", err);
	    else 	
		wrapfs_copy_attr_times(file->f_path.dentry->d_parent->d_inode);
