...
/* global flag indicates -o mmap option */
int MMAP_FLAG = 0;
/*
...
/* parse option for -o mmap */
static int parse_opt(char *options)
{
    	int err = 0;
    	char *opt = "mmap";
    	...
    	if (!options || options[0] == '\0') 
	    	MMAP_FLAG = 0;
    	else if (strlen(options) != strlen(opt))
	    	err = -EINVAL;
    	else if (!memcmp(options,opt,strlen(opt)))
	    	MMAP_FLAG = 1;
	else
		 err = -EINVAL;
	return err;
			    ...
			    const char *dev_name, void *raw_data)
{
	void *lower_path_name = (void *) dev_name;
    	if (parse_opt(raw_data) != 0){
	    pr_err("INVALID ARGUMENT. Set to default.");
	    /* if wrong option for mmap, set to default (use vm_ops) */
	    MMAP_FLAG = 0;
	}
