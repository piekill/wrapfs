...
/* the following code is used for wrapfs crypto */
#define MY_MACIG 'G'
#define RESET_IOCTL _IOW(MY_MACIG, 0, int)
#define WRITE_IOCTL _IOW(MY_MACIG, 1, int)
#define AES_KEYLEN 32
/* flag to indicate -o mmap option */
extern int MMAP_FLAG; 
/* crypto.c */
extern struct blkcipher_desc *desc;
extern int init_cipher(void *key);
extern int wrapfs_encrypt(void *key, void *dst, const void *src, size_t len);
extern int wrapfs_decrypt(void *key, void *dst, const void *src, size_t len);
...
/* code comes from old unionfs, a helper routine to 
 copy a/m/ctime from the lower branch with the newest times */
static inline void wrapfs_copy_attr_times(struct inode *upper)
{
    struct inode *lower;
    
    if (!upper)
	return;
    lower = wrapfs_lower_inode(upper);
    
    if (!lower)
    	return; /*lower dir objects not exist */
